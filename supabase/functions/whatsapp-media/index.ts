...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (dbError || !row || !['document', 'audio', 'image'].includes(row.tipo) || !row.texto?.startsWith('__mired_media__'))
    ...
    if ((row.tipo === 'audio') !== (media.kind === 'audio') || (row.tipo === 'image') !== (media.kind === 'image'))
      return json({ error: 'Tipo de archivo no válido.' }, 400);
    ...
    return new Response(fileResponse.body, {
      ...
      headers: { ...cors, 'Content-Type': meta.mime_type || 'application/octet-stream',
        'Content-Disposition': `${['audio', 'image'].includes(row.tipo) ? 'inline' : 'attachment'}; filename="${name.replace(/[^\x20-\x7e]/g, '_')}"`,
