(function(){
  ...
  function ensureStyles(){
    ...
    const st=document.createElement('style');st.id='cc-performance-styles';
    st.textContent=`
      ...
      .cc-admin{margin-top:10px;padding-top:10px;border-top:1px solid #d8e2ee;display:grid;gap:7px}.cc-admin label{font-size:11px;color:var(--m)}.cc-admin select,.cc-admin input{padding:8px;border:1px solid #cfdceb;border-radius:9px;width:100%}.cc-admin-row{display:grid;grid-template-columns:1fr 90px;gap:7px}
  ...
  function ensureHost(){
    ...
    if(!host){host=document.createElement('div');host.id='ccPerformanceCard';host.className='card cc-wrap';const kpis=dashboard.querySelector('.kpis');dashboard.insertBefore(host,kpis||dashboard.firstChild)}
    return host;
  }
  ...
  function statusHtml(x,title,adminHtml=''){
    ...
    return `<div class="cc-status ${st.key}"><div class="cc-status-top"><div><b>${esc(title)}</b><div class="cc-muted">${esc(x?.asesor_nombre||'Sin asesor')} · Ext. ${shortExt(x?.extension)}</div></div><div class="cc-icon">${st.icon}</div></div><div class="cc-sales">${sales} <span class="cc-muted">ventas</span></div><b>${esc(st.label)}</b><div class="cc-muted">${detail} Meta: ${meta}</div><div class="cc-progress"><span style="width:${pct}%"></span></div>${adminHtml}</div>`;
  }
  function adminControls(x,advisors){
    ...
    return `<div class="cc-admin"><label>Asignar asesor</label><div class="cc-admin-row"><select id="cc-user-${x.extension}">${opts}</select><input id="cc-meta-${x.extension}" type="number" min="1" value="${Number(x.meta_mensual||META_DEFAULT)}" title="Meta mensual"></div><button class="btn ghost" type="button" onclick="window.saveCallCenterExtension('${x.extension}')">Guardar asignación</button></div>`;
  }
  async function saveAssignment(extension){
    const sel=document.getElementById('cc-user-'+extension),metaEl=document.getElementById('cc-meta-'+extension);if(!sel||!metaEl)return;
    const userId=sel.value||null,meta=Math.max(1,Number(metaEl.value||META_DEFAULT));
    const {data:advisor}=userId?await supabaseClient.from('perfiles').select('user_id,nombre,email').eq('user_id',userId).maybeSingle():{data:null};
    const name=advisor?.nombre||null;
    const {error}=await supabaseClient.from('callcenter_extensiones').update({asesor_user_id:userId,asesor_nombre:name,meta_mensual:meta,actualizado_en:new Date().toISOString()}).eq('extension',extension);
    if(error){alert('No se pudo guardar la asignación. '+error.message);return}
    await supabaseClient.from('bases_llamadas').update({asesor_user_id:userId,asesor_nombre:name,actualizado_en:new Date().toISOString()}).eq('extension',extension);
    await loadPerformance();
    alert('Extensión '+shortExt(extension)+' actualizada correctamente.');
  }
